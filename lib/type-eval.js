...
var SOLVERS = {
    ...
    BinaryExpression: function (node, stack) {
        ...
        if (op === '&&' || op === '||') {
            if (left.isConstant()) {
                var leftConst = left.getConstant();
                if (op === '&&' && !leftConst) {
                    return leftConst;
                }
                if (op === '||' && !!leftConst) {
        ...
        //
        var right = solveExpression(node.right, stack);
        ...
        if (op === '+') {
            ...
            if (left.isNot('number') || right.isNot('number')) {
                ...
                return left.isNot('number') ? left : right;
            }
    ...
    ObjectExpression: function (node, stack) {
        ...
        node.properties.forEach(function (node, idx) {
            ...
            values[idx] = solveExpression(node.value, stack, FOLD_CONSTANTS);
        });
    ...
    CallExpression: function (node, stack) {
        var thisObj, fun,
            args = solveExpressions(
                node.arguments, stack,
                FOLD_CONSTANTS
            );
