...
exports.createArray = function (values) {
    var arr = TypeArray.clone(),
        elements = null,
        constant = [];

    // TODO tuples
    // TODO periodic arrays
    // Both are the same, except tuples have fixed length
    for (var i = 0, n = values.length; i < n; i++) {
        var value = values[i];
        if (elements) {
            elements = elements.union(value);
        }
        else {
            elements = value;
        }

        if (!constant || !value.isConstant()) {
            constant = null;
            continue;
        }

        constant[i] = value.getConstant();
    }

    arr.elements = elements;
    return constant
         ? arr.toConstant(constant)
         : arr;
};
